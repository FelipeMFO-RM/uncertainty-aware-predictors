...
\end{itemize}
This implies that prediction errors do not stay local: they \textbf{propagate along the chain}, and any serious controller must reason about that propagation explicitly.
...
This project is orchestrated by a senior LLM (Claude Opus). Implementation tasks --- data wrangling, AutoGluon configurations, training loops, evaluation scripts --- are delegated to a coding LLM (Claude Sonnet / Claude Code in VSCode). The present document is the reference specification for both.
...
\begin{itemize}
    ...
    separates them, enabling decisions like ``collect more data here'' vs.
    ``accept the process noise here''.
    \item \textbf{Asymmetric and probabilistic decisions.} A controller that
    needs $\Pr(y \geq y_{\text{target}}) \geq 0.9$ cannot extract that
    probability from MAE alone. It can from a distribution.
\end{itemize}
...
\begin{enumerate}
    ...
    at $\alpha = 0.9$ (after scalar recalibration if needed).
\end{enumerate}
...
\begin{itemize}
    ...
    \item $\theta$: parameters of a model (the splits of a tree, the weights of a gradient-boosted ensemble); $\Theta$ is the set of all admissible models.
...
\begin{equation}
\mathrm{score}(P) \;=\; \Pr\big(\text{success}\big) \;-\; \lambda \cdot n \;-\; \gamma \cdot \!\!\sum_{i=0}^{n-1} s_i,
\label{eq:risk-score}
\end{equation}
generalizing the deterministic controller (set all variances to zero $\Rightarrow$ $\Pr \in \{0,1\}$ $\Rightarrow$ original boolean check).

\subsection{Annealing: same machinery, simpler topology}

For annealing, the controller is a 2D grid search over $(T, t)$. At each grid point, Monte Carlo propagates the predictive distribution of grain size into IACS and UTS surrogates, yielding a probability of joint feasibility. The optimization criterion becomes: minimize cost $J(T, t)$ subject to $\Pr(\text{success}) \geq p_{\min}$ (e.g., $p_{\min} = 0.9$).
...
\begin{itemize}
    \item Annealing: replace boolean feasibility by $\Pr(\text{success}) \geq p_{\min}$ at each grid point; pick the cheapest feasible.
...
\begin{description}[style=nextline,labelwidth=4cm,leftmargin=4cm,labelsep=0.5cm]
    ...
    \item[$\Pr(\text{success})$] The probability that a candidate process configuration meets all final-property targets. Replaces boolean feasibility in the controller.
